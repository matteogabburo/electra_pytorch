...
class MyRecorder(Recorder):
    ...
    def __init__(
        ...
        train_do_metric=False,
        show_valid_loss=True,
        valid_do_metric=True,
        show_time=True,
        beta=0.98,
    ):
        assert (
        ...
        store_attr(
            self,
            "show_train_loss,train_do_metric,show_valid_loss,valid_do_metric,show_time,beta",
        )
    ...
    def header(self):
        header = L(["epoch"])
        if self.show_train_loss:
            header += "train_loss"
        if self.train_do_metric:
            header += self.metrics.attrgot("name")
        if self.show_valid_loss:
            header += "valid_loss"
    ...
    def after_pred(self):
        if len(self.yb) == 0:
            return
        if (self.training and self.train_do_metric) or (
            not self.training and self.valid_do_metric
        ):
    ...
    def after_train(self):
        if self.show_train_loss:
            self.log += _maybe_item(self.train_loss)
        if self.train_do_metric:
            self.log += self._metrics.map(_maybe_item)

    def after_validate(self):
        if self.show_valid_loss:
            self.log += _maybe_item(self.valid_loss)
        if self.valid_do_metric:
            self.log += self._metrics.map(_maybe_item)
    ...
    @property
    def _metrics(self):
        if self.training and getattr(self, "cancel_train", False):
            return L()
        if not self.training and getattr(self, "cancel_valid", False):
            return L()
        return self.metrics
...
class MulitTaskRecorder(MyRecorder):
    ...
    @property
    def valid_loss(self):
        return self.valid_losses[self.current_task_idx]

    def dont_measure(self, i):
        return (
            i in self.tasks_dont_measure
            or self.task_names[i] in self.tasks_dont_measure
        )

    def dont_metric(self, i):
        return (
            i in self.tasks_dont_metric or self.task_names[i] in self.tasks_dont_metric
        )
    ...
    def header(self):
        ...
        if self.train_do_metric:
            for i, metrics in enumerate(self.multi_metrics):
                if self.dont_measure(i) or self.dont_metric(i):
                    continue
                for metric in metrics:
                    header += f"{self.task_names[i]}:{metric.name}"
        if self.show_valid_loss or self.valid_do_metric:
            for i, metrics in enumerate(self.multi_metrics):
                if self.dont_measure(i):
                    continue
                if self.show_valid_loss:
                    header += f"{self.task_names[i]}:valid_loss"
                if self.dont_metric(i):
                    continue
                if self.valid_do_metric:
                    for metric in metrics:
                        header += f"{self.task_names[i]}:{metric.name}"
...
class MultiTaskDataloader:
    ...
    def __len__(self):
        return max([len(dl) for dl in self.multi_dl])
    ...
    def __iter__(self):
        ...
        """
        if self.cycle:
            return cycle([cycle(dl) for dl in self.multi_dl])
        else:
            return cycle([not_cycle_infinite(dl) for dl in self.multi_dl])
...
class MultiTaskDataloaders:
    def __init__(self, multi_dls, cycle=False):
        ...
        self.train = MultiTaskDataloader(
            [dls.train for dls in self.multi_dls], cycle=cycle
        )

    def __getitem__(self, i):
        return self if i is None else self.multi_dls[i]

    def __len__(self):
        return len(self.multi_dls)
...
class MultiTaskLearner(MyLearner):
    def __init__(
        ...
        store_attr(
            self,
            "multi_model,task_weights,task_names,multi_cbs,multi_metrics,tasks_dont_measure,opt_func,lr,splitter,model_dir,wd,wd_bn_bias,train_bn,moms",
        )
        # infer
        self.multi_loss_func = self.infer_loss_func(multi_loss_func)
        if task_weights is None:
            self.task_weights = [1.0] * len(self)
        if task_names is None:
            self.task_names = [f"t{i}" for i in range(len(self))]
        if cbs is None:
            cbs = []
        if multi_cbs is None:
            self.multi_cbs = [[] for _ in range(len(self))]
        if multi_metrics is None:
            self.multi_metrics = [[] for _ in range(len(self))]
        self.path = Path(path) if path is not None else getattr(dls, "path", Path("."))
        # check
        assert (
            len(self)
            == len(self.multi_dls)
            == len(self.multi_model)
            == len(self.multi_loss_func)
            == len(self.task_weights)
            == len(self.task_names)
            == len(self.multi_cbs)
        ...
        self.add_cbs(
            [
                (cb() if isinstance(cb, type) else cb)
                for cb in [
                    TrainEvalCallback(),
                    MulitTaskRecorder(),
                    MyMyProgressCallback,
                    *cbs,
                ]
            ]
        )
    ...
    @property
    def model(self):
        return self.multi_model.switch(self.current_task_idx)
    ...
    @property
    def dls(self):
        return self.multi_dls[self.current_task_idx]

    @property
    def loss_func(self):
        return self.multi_loss_func[self.current_task_idx]

    def infer_loss_func(self, multi_loss_func):
        if multi_loss_func is None:
            multi_loss_func = [
                getattr(dls.train_ds, "loss_func", None) for dls in self.multi_dls
            ]
        else:
            assert len(multi_loss_func) == len(self)
            for i, loss_func in enumerate(multi_loss_func):
                if loss_func is None:  # then infer from dls
                    multi_loss_func[i] = getattr(
                        self.multi_dls[i].train_ds, "loss_func", None
                    )
        fails = [
            str(i) for i, loss_func in enumerate(multi_loss_func) if loss_func is None
        ]
        assert (
            not fails
        ), f"Could not infer loss function from the dataloaders for task {', '.join(fails)}, please pass loss functions for them."
    ...
    def _call_one(self, event_name):
        ...
        local_cbs = (
            ...
            else self.multi_cbs[self.current_task_idx]
        )
    ...
    def one_loss(self):
        ...
        losses = []
        for i, dl_iter in zip(
            range(len(self)), self.dl_iter
        ):  # here self.dl_iter is actually dl_iters
            ...
            losses.append(loss)
        self.current_task_idx = None
        self.loss = self.merge_loss(losses)
        self("after_loss")
    ...
    def merge_loss(self, losses):
        ...
        for l in losses:
            if isinstance(l, torch.Tensor):
                l_dtype, l_device = l.dtype, l.device
                break
        # convert all fake loss (0) to tensor
        for i, l in enumerate(losses):
            if l is None:
                losses[i] = torch.tensor(0, dtype=l_dtype, device=l_device)
        losses_tensor = torch.stack(losses)
        task_weights_tensor = torch.tensor(
            self.task_weights, dtype=l_dtype, device=l_device
        )
    ...
    def _do_epoch_validate(self):
        for self.current_task_idx in range(len(self)):
            if self.current_task_idx in self.tasks_dont_measure:
                continue
            if self.task_names[self.current_task_idx] in self.tasks_dont_measure:
                continue
