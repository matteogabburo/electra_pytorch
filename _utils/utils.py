...
def Adam_no_bias_correction(
    params, lr, mom=0.9, sqr_mom=0.99, eps=1e-5, wd=0.01, decouple_wd=True
):
...
def linear_warmup_and_decay(
    ...
    decayed_lr = (lr_max - end_lr) * (
        1 - step_i / total_steps
    ) ** decay_power + end_lr  # https://www.tensorflow.org/api_docs/python/tf/compat/v1/train/polynomial_decay
    warmed_lr = decayed_lr * min(
        1.0, step_i / warmup_steps
    )  # https://github.com/google-research/electra/blob/81f7e5fc98b0ad8bfd20b641aa8bc9e6ac00c8eb/model/optimization.py#L44
    return warmed_lr


def linear_warmup_and_then_decay(
    pct,
    lr_max,
    total_steps,
    warmup_steps=None,
    ...
    if step_i <= warmup_steps:  # warm up
        ...
        return (lr_max - end_lr) * (
            1 - (step_i - warmup_steps) / (total_steps - warmup_steps)
        ) ** decay_power + end_lr
...
class ConcatTransform(CombineTransform):
    def __init__(
        self, hf_dset, hf_tokenizer, max_length, text_col="text", book="multi"
    ):
        super().__init__(
            hf_dset, in_cols=[text_col], out_cols=["input_ids", "sentA_length"]
        )
    ...
    def accumulate(self, sentence):
        if "isbn" in sentence:
            return
        tokens = self.hf_tokenizer.convert_tokens_to_ids(
            self.hf_tokenizer.tokenize(sentence)
        )
        tokens = tokens[: self.max_length - 2]  # trim sentence to max length if needed
        if (
            self.book == "single"
            or (len(self.input_ids) + len(tokens) + 1 > self.max_length)
            or (self.book == "bi" and len(self.sent_lens) == 2)
        ):
    ...
    def create_example(self):
        ...
        if self.book == "multi":
            ...
            for i in range(len(self.sent_lens)):
                current_diff = abs(
                    sum(self.sent_lens[: i + 1]) - sum(self.sent_lens[i + 1 :])
                )
            ...
            return {
                ...
                "sentA_length": sum(self.sent_lens[:i]),
            }
        else:
...
class ELECTRADataProcessor(object):
    ...
    def _make_example(self, first_segment, second_segment):
        ...
        input_ids = (
            ...
            + first_segment
            + [self.hf_tokenizer.sep_token_id]
        )
