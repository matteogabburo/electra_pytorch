...
def wsc_trick_process(sample, hf_toker):
    ...
    sample["cands"] = reduce(
        operator.add, cands, []
    )  # flatten list, into a 1d token ids
...
class ELECTRAWSCTrickModel(nn.Module):
    ...
    def forward(self, *xb):
        ...
        for i in range(batch_size):
            ...
            cands = cands.split(
                cand_lens.tolist()
            )  # split into list of tensors, i th has length of cand_lens[i]
            ...
            for cand in cands:
                ...
                sents.append(
                    torch.cat(
                        [prefix, cand, suffix, cand.new_full((pad_len,), self.pad_idx)]
                    )
                )
                masks.append(
                    torch.cat(
                        [
                            cand.new_zeros(len(prefix)),
                            cand.new_ones(len(cand)),
                            cand.new_zeros(max_len - len(prefix) - len(cand)),
                        ]
                    )
                )
        ...
        n_cands = torch.tensor(
            n_cands, device=all_scores.device
        )  # (B,), number of candidates for each sample
...
class ELECTRAWSCTrickLoss:
    ...
    def __call__(self, x, y):
        ...
        for scores, label in zip(
            all_scores.split(n_cands.tolist()), y
        ):  # for every sample
            if label == 0:
                continue  # only calculate loss on positive samples
            labels = [0.0] + [1.0] * (len(scores) - 1)  # labels for BCEWithLogitLoss
    ...
    def decodes(self, preds):
        return wsc_trick_predict(preds)
...
def get_wsc_trick_processing(hf_tokenizer):
    ...
    def wsc_trick_process(sample):
        ...
        target_start, target_end = sample["span2_index"], sample["span2_index"] + len(
            sample["span2_text"].split()
        )
        ...
        cands = [simple_encode(sample["span1_text"])] + [
            simple_encode(cand) for cand in cands
        ]
        sample["cands"] = reduce(
            operator.add, cands, []
        )  # flatten list, into a 1d token ids
