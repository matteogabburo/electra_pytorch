...
if not c.base_run_name:
    c.base_run_name = (
        str(datetime.now(timezone(timedelta(hours=+8))))[6:-13]
        .replace(" ", "")
        .replace(":", "")
        .replace("-", "")
    )
...
gen_config.num_attention_heads = (
    disc_config.num_attention_heads // generator_size_divisor
)
...
if "wikipedia" in c.datas:
    ...
    wiki = datasets.load_dataset("wikipedia", "20200501.en", cache_dir="./datasets")[
        "train"
    ]
    print("load/create data from wiki dataset for ELECTRA")
    e_wiki = ELECTRAProcessor(wiki).map(
        cache_file_name=f"electra_wiki_{c.max_length}.arrow", num_proc=1
    )
    dsets.append(e_wiki)
...
def mask_tokens(
    ...
    * ignore_index in nn.CrossEntropy is default to -100, so you don't need to specify ignore_index in loss
    ...
    if int(replace_prob) != 0:
        ...
        replace_token_mask = (
            torch.bernoulli(torch.full(labels.shape, rep_prob, device=device)).bool()
            & mlm_mask
            & ~mask_token_mask
        )
...
class MaskedLMCallback(Callback):
    ...
    def show_batch(self, dl, idx_show_ignored, verbose=True, **kwargs):
        ...
        assert torch.equal(
            (~is_mlm_applied * masked_inputs + is_mlm_applied * labels), input_ids
        )
        ...
        # show
        tfm_b = (
            (masked_inputs, sentA_lenths, is_mlm_applied, labels)
            if self.for_electra
            else (masked_inputs, sentA_lenths, labels)
        )
        dl.show_batch(b=tfm_b, **kwargs)


# %%
mlm_cb = MaskedLMCallback(
    ...
    def forward(self, masked_inputs, sentA_lenths, is_mlm_applied, labels):
        ...
        if c.my_model:
            gen_logits = self.generator(
                masked_inputs, attention_mask, token_type_ids, is_mlm_applied
            )[0]
...
class ELECTRALoss:
    def __init__(
        ...
        self.gen_loss_fc = (
            LabelSmoothingCrossEntropyFlat(eps=gen_label_smooth)
            if gen_label_smooth
            else CrossEntropyLossFlat()
        )
...
if c.adam_bias_correction:
    ...
    opt_func = partial(
        Adam_no_bias_correction, eps=1e-6, mom=0.9, sqr_mom=0.999, wd=0.01
    )
...
if c.logger == "neptune":
    ...
    wandb.init(
        name=c.run_name, project="electra_pretrain", config={**c, **hparam_update}
    )
    learn.add_cb(WandbCallback(log_preds=False, log_model=False))

# Mixed precison and Gradient clip
learn.to_native_fp16(init_scale=2.0 ** 11)
