...
class WSCTransform(HF_BaseTransform):
    ...
    def __call__(self, sample):
        ...
        prefix = self.tokenizer.encode(" ".join(sample["text"].split()[:target_start]))[
            :-1
        ]  # no sep
        suffix = self.tokenizer.encode(" ".join(sample["text"].split()[target_end:]))[
            1:
        ]  # no cls
        ...
        sample["span"] = [
            len(prefix),
            len(prefix) + len(candidate),
        ]  # start and end of candidate
        return sample
...
class ELECTRAWSCModel(nn.Module):
    ...
    def forward(self, x, spans):
        ...
        masks = torch.stack(
            [
                x.new_zeros(x.shape[1]).scatter(
                    0, torch.arange(start, end, device=x.device), 1
                )
