...
class Timer(RunSteps):
    ...
    def __init__(
        self, n_steps, ignore_first_n=1, break_after=None, precision=3, **kwargs
    ):
        """
        Args:
    ...
    def before_batch(self):
        ...
        if self.break_after == "before_batch":
            raise CancelBatchException

    def after_pred(self):
        self.times[1].append(self.time_delta())
        if self.break_after == "after_pred":
            raise CancelBatchException

    def after_loss(self):
        self.times[2].append(self.time_delta())
        if self.break_after == "after_loss":
            raise CancelBatchException

    def after_backward(self):
        self.times[3].append(self.time_delta())
        if self.break_after == "after_backward":
            raise CancelBatchException

    def after_step(self):
        self.times[4].append(self.time_delta())
        if self.break_after == "after_step":
            raise CancelBatchException
    ...
    def show(self):
        print(
            f"show average and standard deviation of step {self.ignore_first_n+1} ~ step {self.train_iter} (total {self.steps-self.ignore_first_n} training steps)"
        )
        # print for each stage
        for i, deltas in enumerate(self.times):
            if len(deltas) == 0:
                time_message = (
                    "Skipped or Exception raised by callbacks ran before Timer."
                )
            else:
                m, s = mean(deltas[self.ignore_first_n :]), stdev(
                    deltas[self.ignore_first_n :]
                )
                time_message = f"avg {round(m, self.precision)} secs ± stdev {round(s, self.precision)}"
            print(f"{(_MESSAGE[i]+':'):36} {time_message}")
        # print for total
        times = list([_f for _f in self.times if _f])
        ...
        for i, deltas in enumerate(times):
            if len(deltas) < max_len:
                times[i] += [0] * (max_len - len(deltas))
        ## calculate
        times = torch.tensor(times)[:, self.ignore_first_n :]
        total_m, total_s = times.sum(0).mean().item(), times.sum(0).std().item()
        print(
            f"Total: avg {round(total_m, self.precision)} secs ± stdev {round(total_s, self.precision)} secs"
        )
